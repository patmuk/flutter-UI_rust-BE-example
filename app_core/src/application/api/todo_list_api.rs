...
use crate::application::api::lifecycle::{self, APP_STATE};
...
pub fn process_command(command: Command) -> Vec<Effect> {
    ...
    let effects = process_command_todo_list(command, &mut APP_STATE.write().model);
...
pub fn process_query(query: Query) -> Vec<Effect> {
    ...
    let effects = process_query_todo_list(query, &APP_STATE.read().model);
